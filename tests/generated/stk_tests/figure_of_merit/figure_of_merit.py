...
class EarlyBoundTests(TestBase):
    ...
    def test_Basic(self):
        ...
        while iIndex < len(arTypes):
            ...
            if eType == FM_DEFINITION_TYPE.ACCESS_CONSTRAINT:
                ...
                fdac = EarlyBoundTests.AG_FOM.set_access_constraint_definition(FM_CONSTRAINT_NAME.AZIMUTH_RATE)
            ...
            eType2: "FM_DEFINITION_TYPE" = EarlyBoundTests.AG_FOM.definition_type
    ...
    def test_BUG72717(self):
        ...
        gfxAnim: "IFigureOfMeritGfxAttributesAnimation" = gfx.animation
        gfxStatic: "IFigureOfMeritGfxAttributes" = gfx.static
    ...
    def test_DefinitionTypes(self):
        defType: "FM_DEFINITION_TYPE"
        for defType in Enum.GetValues(clr.TypeOf(FM_DEFINITION_TYPE)):
            if EarlyBoundTests.AG_FOM.is_definition_type_supported(defType):
                if defType == FM_DEFINITION_TYPE.ACCESS_CONSTRAINT:
                    EarlyBoundTests.AG_FOM.set_access_constraint_definition(FM_CONSTRAINT_NAME.ALTITUDE)
                    Assert.assertEqual(FM_DEFINITION_TYPE.ACCESS_CONSTRAINT, EarlyBoundTests.AG_FOM.definition_type)

                elif defType == FM_DEFINITION_TYPE.SCALAR_CALCULATION:
                    EarlyBoundTests.AG_FOM.set_scalar_calculation_definition("CentralBody/Earth ElapsedTimeFromStart")
                    Assert.assertEqual(FM_DEFINITION_TYPE.SCALAR_CALCULATION, EarlyBoundTests.AG_FOM.definition_type)
    ...
    def test_GridInspector(self):
        ...
        TestBase.logger.WriteLine6("\tThe current DefinitionType is: {0}", EarlyBoundTests.AG_FOM.definition_type)
        EarlyBoundTests.AG_FOM.set_definition_type(FM_DEFINITION_TYPE.N_ASSET_COVERAGE)
        TestBase.logger.WriteLine6("\tThe new DefinitionType is: {0}", EarlyBoundTests.AG_FOM.definition_type)
        # Definition
        ...
        # PointFOM
        oTimeVar: "IDataProviderTimeVarying" = clr.Convert(oInspector.point_fom, IDataProviderTimeVarying)
        ...
        # RegionFOM
        oTimeVar = clr.Convert(oInspector.region_fom, IDataProviderTimeVarying)
        Assert.assertIsNotNone(oTimeVar)
        oResult = DataProviderResultWriter(oTimeVar.exec_single("1 Jul 1999 00:00:00.00"))
        TestBase.logger.WriteLine("\n\tRegionFOM result:")
        oResult.Dump()
        ...
        # PointFOM
        oTimeVar = clr.Convert(oInspector.point_fom, IDataProviderTimeVarying)
        Assert.assertIsNotNone(oTimeVar)
        oResult = DataProviderResultWriter(oTimeVar.exec_single("1 Jul 1999 00:00:00.00"))
        TestBase.logger.WriteLine("\n\tPointFOM result:")
        oResult.Dump()
    ...
    def test_Graphics(self):
        ...
        # Contours (readonly)
        EarlyBoundTests.AG_FOM.set_definition_type(FM_DEFINITION_TYPE.ACCESS_SEPARATION)
        Assert.assertEqual(FM_DEFINITION_TYPE.ACCESS_SEPARATION, EarlyBoundTests.AG_FOM.definition_type)
        helper.GfxContours(oGraphics.static.contours, True, True)
        helper.GfxContourLines(oGraphics.static.contours, True)
        # Contours
        EarlyBoundTests.AG_FOM.set_definition_type(FM_DEFINITION_TYPE.TIME_AVERAGE_GAP)
        Assert.assertEqual(FM_DEFINITION_TYPE.TIME_AVERAGE_GAP, EarlyBoundTests.AG_FOM.definition_type)
        ...
        # Animation
        EarlyBoundTests.AG_FOM.set_definition_type(FM_DEFINITION_TYPE.SIMPLE_COVERAGE)
        Assert.assertEqual(FM_DEFINITION_TYPE.SIMPLE_COVERAGE, EarlyBoundTests.AG_FOM.definition_type)
        ...
        # Contours
        EarlyBoundTests.AG_FOM.set_definition_type(FM_DEFINITION_TYPE.REVISIT_TIME)
        Assert.assertEqual(FM_DEFINITION_TYPE.REVISIT_TIME, EarlyBoundTests.AG_FOM.definition_type)
    ...
    def test_VO(self):
        ...
        # VO
        oVO: "IFigureOfMeritVO" = EarlyBoundTests.AG_FOM.vo
    ...
    # region VOAttributes
    def VOAttributes(self, oAttributes: "IFigureOfMeritVOAttributes", bReadOnly: bool):
    ...
    def test_GfxSmoothContours(self):
        ...
        try:
            ...
            noSmoothFillBounds: "CV_BOUNDS[]" = [CV_BOUNDS.BOUNDS_CUSTOM_BOUNDARY]
            eBound: "CV_BOUNDS"
            for eBound in noSmoothFillBounds:
                ...
                eBound2: "CV_BOUNDS" = oCovDef.grid.bounds_type
                Assert.assertEqual(eBound, eBound2)
                self.TestFOMGfxContours(oFOMerit, False)

            SmoothFillBounds: "CV_BOUNDS[]" = [
                CV_BOUNDS.BOUNDS_LAT_LINE,
                CV_BOUNDS.BOUNDS_LON_LINE,
                CV_BOUNDS.BOUNDS_CUSTOM_REGIONS,
                CV_BOUNDS.BOUNDS_GLOBAL,
                CV_BOUNDS.BOUNDS_LAT,
            ]
            eBound: "CV_BOUNDS"
    ...
    def TestFOMGfxContours(self, fom: "IFigureOfMerit", bIsSmoothFillSupported: bool):
        ...
        Assert.assertEqual(FM_DEFINITION_TYPE.ACCESS_DURATION, fom.definition_type)
        ...
        if not bIsSmoothFillSupported:
            ...
            def action14():
                fom.graphics.static.contours.contour_type = FM_GFX_CONTOUR_TYPE.SMOOTH_FILL
            ...
            Assert.assertEqual(FM_GFX_CONTOUR_TYPE.SMOOTH_FILL, fom.graphics.static.contours.contour_type)
    ...
    def test_NonLinearContourLevels(self):
        ...
        covdef.point_definition.grid_class = CV_GRID_CLASS.GRID_CLASS_RECEIVER
        ...
        if not TestBase.NoGraphicsMode:
            fomc1: "IFigureOfMeritGfxContours" = fom1.graphics.static.contours
            fomc1.is_visible = True
            col1: "IFigureOfMeritGfxLevelAttributesCollection" = fomc1.level_attributes
        ...
        if not TestBase.NoGraphicsMode:
            fomc2: "IFigureOfMeritGfxContours" = fom2.graphics.static.contours
            fomc2.is_visible = True
            col2: "IFigureOfMeritGfxLevelAttributesCollection" = fomc2.level_attributes
    ...
    def test_AccessConstraintDefinitions(self):
        ...
        try:
            fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.LINE_OF_SIGHT)
        ...
        try:
            fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.ACCESS_CONSTRAINT_PLUGIN)
        ...
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.FREQUENCY)
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.ALTITUDE)
        Assert.assertEqual(FM_CONSTRAINT_NAME.ALTITUDE, fomcs.constraint_name)
        ...
        # Set to other than the access constraint we are going to test.
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.FREQUENCY)
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.BER_PLUS_I)
        Assert.assertEqual(FM_CONSTRAINT_NAME.BER_PLUS_I, fomcs.constraint_name)
        ...
        # Set to other than the access constraint we are going to test.
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.FREQUENCY)
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.OVER_NO)
        Assert.assertEqual(FM_CONSTRAINT_NAME.OVER_NO, fomcs.constraint_name)
        ...
        # Set to other than the access constraint we are going to test.
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.FREQUENCY)
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.J_OVER_S)
        Assert.assertEqual(FM_CONSTRAINT_NAME.J_OVER_S, fomcs.constraint_name)
        ...
        # Set to other than the access constraint we are going to test.
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.FREQUENCY)
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.SRCH_TRK_SINGLE_PULSE_J_OVER_S)
        Assert.assertEqual(FM_CONSTRAINT_NAME.SRCH_TRK_SINGLE_PULSE_J_OVER_S, fomcs.constraint_name)
        ...
        # Set to other than the access constraint we are going to test.
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.FREQUENCY)
        fomcs = fom.set_access_constraint_definition(FM_CONSTRAINT_NAME.CRDN_CONDITION)
        Assert.assertEqual(FM_CONSTRAINT_NAME.CRDN_CONDITION, fomcs.constraint_name)
