...
class EarlyBoundTests(TestBase):
    ...
    def test_STKObject(self):
        ...
        EarlyBoundTests.AG_COV.grid.bounds_type = COVERAGE_BOUNDS.LATITUDE
    ...
    def test_ComputeAccess(self):
        ...
        polarSat.set_propagator_type(PROPAGATOR_TYPE.J4_PERTURBATION)
        ...
        classical.location_type = CLASSICAL_LOCATION.TRUE_ANOMALY
        trueAnomaly: "ClassicalLocationTrueAnomaly" = ClassicalLocationTrueAnomaly(classical.location)
        trueAnomaly.value = 0.0
        classical.size_shape_type = CLASSICAL_SIZE_SHAPE.ALTITUDE
        ...
        if not TestBase.NoGraphicsMode:
            polarSat.graphics.set_attributes_type(VEHICLE_GRAPHICS_2D_ATTRIBUTE_TYPE.BASIC)
        ...
        j4.propagate()
        if not TestBase.NoGraphicsMode:
            shuttle.graphics.set_attributes_type(VEHICLE_GRAPHICS_2D_ATTRIBUTE_TYPE.BASIC)
            basicAtt: "IVehicleGraphics2DAttributesBasic" = IVehicleGraphics2DAttributesBasic(
        ...
        if not TestBase.NoGraphicsMode:
            ...
            twoEyes.graphics.animation_settings.accumulation = FIGURE_OF_MERIT_GRAPHICS_2D_ACCUMULATION.CURRENT_TIME
        ...
        if not TestBase.NoGraphicsMode:
            ...
            twoEyes.graphics.animation_settings.contours.color_method = (
    ...
    def test_Grid(self):
        ...
        Assert.assertEqual(COVERAGE_BOUNDS.GLOBAL, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.GLOBAL)

        # BoundsType (LATITUDE)
        oGrid.bounds_type = COVERAGE_BOUNDS.LATITUDE
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", oGrid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.LATITUDE, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.LATITUDE)

        # BoundsType (LATITUDE_LINE)
        oGrid.bounds_type = COVERAGE_BOUNDS.LATITUDE_LINE
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", oGrid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.LATITUDE_LINE, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.LATITUDE_LINE)

        # BoundsType (LONGITUDE_LINE)
        oGrid.bounds_type = COVERAGE_BOUNDS.LONGITUDE_LINE
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", oGrid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.LONGITUDE_LINE, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.LONGITUDE_LINE)

        # BoundsType (LATITUDE_LONGITUDE_REGION)
        oGrid.bounds_type = COVERAGE_BOUNDS.LATITUDE_LONGITUDE_REGION
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", oGrid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.LATITUDE_LONGITUDE_REGION, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.LATITUDE_LONGITUDE_REGION)

        # BoundsType (CUSTOM_REGIONS)
        oGrid.bounds_type = COVERAGE_BOUNDS.CUSTOM_REGIONS
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", oGrid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.CUSTOM_REGIONS, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.CUSTOM_REGIONS)
        ...
        oGrid.bounds_type = COVERAGE_BOUNDS.CUSTOM_BOUNDARY
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", oGrid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.CUSTOM_BOUNDARY, oGrid.bounds_type)
        self.Bounds(oGrid.bounds, COVERAGE_BOUNDS.CUSTOM_BOUNDARY)
        ...
        TestBase.logger.WriteLine6("\tThe new ResolutionType is: {0}", oGrid.resolution_type)
        Assert.assertEqual(COVERAGE_RESOLUTION.RESOLUTION_LATITUDE_LONGITUDE, oGrid.resolution_type)
        self.Resolution(oGrid.resolution, COVERAGE_RESOLUTION.RESOLUTION_LATITUDE_LONGITUDE)
    ...
    def Bounds(self, oBounds: "ICoverageBounds", eType: "COVERAGE_BOUNDS"):
        ...
        if eType == COVERAGE_BOUNDS.CUSTOM_REGIONS:
            ...
            place: "IStkObject" = TestBase.Application.current_scenario.children.new(STK_OBJECT_TYPE.PLACE, "Place")
            ...
            # Add with target
            target: "IStkObject" = TestBase.Application.current_scenario.children.new(STK_OBJECT_TYPE.TARGET, "Target")
        ...
        elif eType == COVERAGE_BOUNDS.LATITUDE_LONGITUDE_REGION:
        ...
        elif eType == COVERAGE_BOUNDS.LATITUDE_LINE:
        ...
        elif eType == COVERAGE_BOUNDS.LONGITUDE_LINE:
        ...
        elif eType == COVERAGE_BOUNDS.CUSTOM_BOUNDARY:
    ...
    def Resolution(self, oResolution: "ICoverageResolution", eType: "COVERAGE_RESOLUTION"):
        ...
        elif eType == COVERAGE_RESOLUTION.RESOLUTION_LATITUDE_LONGITUDE:
    ...
    def test_PointDefinition(self):
        ...
        oPD.point_location_method = COVERAGE_POINT_LOCATION_METHOD.COMPUTE_BASED_ON_RESOLUTION
        TestBase.logger.WriteLine6("\tThe new PointLocationMethod is: {0}", oPD.point_location_method)
        Assert.assertEqual(COVERAGE_POINT_LOCATION_METHOD.COMPUTE_BASED_ON_RESOLUTION, oPD.point_location_method)
        # PointFileList (readonly)
        self.PointFileListCollection(oPD.point_file_list, True)
        # PointLocationMethod (SPECIFY_CUSTOM_LOCATIONS)
        oPD.point_location_method = COVERAGE_POINT_LOCATION_METHOD.SPECIFY_CUSTOM_LOCATIONS
        TestBase.logger.WriteLine6("\tThe new PointLocationMethod is: {0}", oPD.point_location_method)
        Assert.assertEqual(COVERAGE_POINT_LOCATION_METHOD.SPECIFY_CUSTOM_LOCATIONS, oPD.point_location_method)
        # PointFileList
        self.PointFileListCollection(oPD.point_file_list, False)
        # PointLocationMethod (UNKNOWN)
        with pytest.raises(Exception):
            oPD.point_location_method = COVERAGE_POINT_LOCATION_METHOD.UNKNOWN
        ...
        oPD.ground_altitude_method = COVERAGE_GROUND_ALTITUDE_METHOD.ALTITUDE_ABOVE_MEAN_SEA_LEVEL
        Assert.assertEqual(COVERAGE_GROUND_ALTITUDE_METHOD.ALTITUDE_ABOVE_MEAN_SEA_LEVEL, oPD.ground_altitude_method)
    ...
    def GridClass(self, oPD: "CoveragePointDefinition", eClass: "COVERAGE_GRID_CLASS"):
        ...
        if (eClass == COVERAGE_GRID_CLASS.UNKNOWN) or (eClass == COVERAGE_GRID_CLASS.SUBMARINE):
            with pytest.raises(Exception):
        ...
        if (
            ((eClass == COVERAGE_GRID_CLASS.RADAR) or (eClass == COVERAGE_GRID_CLASS.RECEIVER))
            or (eClass == COVERAGE_GRID_CLASS.TRANSMITTER)
        ) or (eClass == COVERAGE_GRID_CLASS.SENSOR):
            with pytest.raises(Exception):
        ...
        oPD.altitude_method = COVERAGE_ALTITUDE_METHOD.ABOVE_ELLIPSOID
        TestBase.logger.WriteLine6("\t\tThe new AltitudeMethod is: {0}", oPD.altitude_method)
        Assert.assertEqual(COVERAGE_ALTITUDE_METHOD.ABOVE_ELLIPSOID, oPD.altitude_method)
        # Altitude
        ...
        oPD.altitude_method = COVERAGE_ALTITUDE_METHOD.ABOVE_MEAN_SEA_LEVEL
        TestBase.logger.WriteLine6("\t\tThe new AltitudeMethod is: {0}", oPD.altitude_method)
        Assert.assertEqual(COVERAGE_ALTITUDE_METHOD.ABOVE_MEAN_SEA_LEVEL, oPD.altitude_method)
        # Altitude
        TestBase.logger.WriteLine6("\t\t\tThe current Altitude is: {0}", oPD.altitude)
        oPD.altitude = 123.456
        TestBase.logger.WriteLine6("\t\t\tThe new Altitude is: {0}", oPD.altitude)
        ...
        TestBase.logger.WriteLine6("\t\tThe current AltitudeMethod is: {0}", oPD.altitude_method)
        oPD.altitude_method = COVERAGE_ALTITUDE_METHOD.ABOVE_TERRAIN
        TestBase.logger.WriteLine6("\t\tThe new AltitudeMethod is: {0}", oPD.altitude_method)
        Assert.assertEqual(COVERAGE_ALTITUDE_METHOD.ABOVE_TERRAIN, oPD.altitude_method)
        # Altitude
        TestBase.logger.WriteLine6("\t\t\tThe current Altitude is: {0}", oPD.altitude)
        if (eClass != COVERAGE_GRID_CLASS.FACILITY) and (eClass != COVERAGE_GRID_CLASS.TARGET):
        ...
        if (eClass == COVERAGE_GRID_CLASS.AIRCRAFT) or (eClass == COVERAGE_GRID_CLASS.SATELLITE):
            TestBase.logger.WriteLine6("\t\tThe current AltitudeMethod is: {0}", oPD.altitude_method)
        ...
        with pytest.raises(Exception):
            oPD.altitude_method = COVERAGE_ALTITUDE_METHOD.UNKNOWN
    ...
    def test_GridInspector(self):
        ...
        Assert.assertEqual(COVERAGE_BOUNDS.LATITUDE, EarlyBoundTests.AG_COV.grid.bounds_type)
        # Bounds
        ...
        # BoundsType (CUSTOM_REGIONS)
        EarlyBoundTests.AG_COV.grid.bounds_type = COVERAGE_BOUNDS.CUSTOM_REGIONS
        TestBase.logger.WriteLine6("\tThe new BoundsType is: {0}", EarlyBoundTests.AG_COV.grid.bounds_type)
        Assert.assertEqual(COVERAGE_BOUNDS.CUSTOM_REGIONS, EarlyBoundTests.AG_COV.grid.bounds_type)
        # Bounds
    ...
    def test_GridPointSelectionIterationVsIndexing(self):
        ...
        EarlyBoundTests.AG_COV.grid.resolution_type = COVERAGE_RESOLUTION.RESOLUTION_LATITUDE_LONGITUDE
    ...
    def test_GridInspectorAllTypes(self):
        ...
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.LATITUDE)  # original
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.CUSTOM_BOUNDARY)
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.CUSTOM_REGIONS)
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.GLOBAL)
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.LATITUDE_LINE)
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.LONGITUDE_LINE)
        self.CompareGridPointsByBoundsType(covDef, COVERAGE_BOUNDS.LATITUDE_LONGITUDE_REGION)

        # restore to original
        covDef.grid.bounds_type = COVERAGE_BOUNDS.LATITUDE
    ...
    def test_Advanced(self):
        ...
        with pytest.raises(Exception):
            oAdvanced.data_retention = COVERAGE_DATA_RETENTION.UNKNOWN
        # SaveMode
        ...
        oAdvanced.region_access_acceleration = COVERAGE_REGION_ACCESS_ACCELERATION_TYPE.AUTOMATIC
        TestBase.logger.WriteLine6("\tThe new RegionAccessAcceleration is: {0}", oAdvanced.region_access_acceleration)
        Assert.assertEqual(COVERAGE_REGION_ACCESS_ACCELERATION_TYPE.AUTOMATIC, oAdvanced.region_access_acceleration)
        oAdvanced.region_access_acceleration = COVERAGE_REGION_ACCESS_ACCELERATION_TYPE.OFF
        TestBase.logger.WriteLine6("\tThe new RegionAccessAcceleration is: {0}", oAdvanced.region_access_acceleration)
        Assert.assertEqual(COVERAGE_REGION_ACCESS_ACCELERATION_TYPE.OFF, oAdvanced.region_access_acceleration)
        with pytest.raises(Exception):
            oAdvanced.region_access_acceleration = COVERAGE_REGION_ACCESS_ACCELERATION_TYPE.UNKNOWN
    ...
    def test_PointAltitude(self):
        ...
        pointDef.altitude_method = COVERAGE_ALTITUDE_METHOD.ABOVE_TERRAIN

        fac: "Facility" = Facility(
            TestBase.Application.current_scenario.children.new(STK_OBJECT_TYPE.FACILITY, "Fac_PointAltitude")
        )
        constraint: "IAccessConstraint" = fac.access_constraints.add_constraint(ACCESS_CONSTRAINT_TYPE.LIGHTING)
        ...
        pointDef.grid_class = COVERAGE_GRID_CLASS.FACILITY
        pointDef.use_grid_seed = True
        pointDef.seed_instance = "Facility/Fac_PointAltitude"

        Assert.assertEqual(COVERAGE_ALTITUDE_METHOD.ABOVE_TERRAIN, pointDef.altitude_method)

        TestBase.logger.WriteLine("----- PointAltitude TEST ----- END -----")

    # endregion

    @parameterized.expand([(COVERAGE_POINT_ALTITUDE_METHOD.FILE_VALUES,), (COVERAGE_POINT_ALTITUDE_METHOD.OVERRIDE,)])
    def test_CustomPointAltitudeMethod(self, method: "COVERAGE_POINT_ALTITUDE_METHOD"):
        ...
        try:
            pointDef: "CoveragePointDefinition" = covDef.point_definition
            pointDef.point_location_method = COVERAGE_POINT_LOCATION_METHOD.SPECIFY_CUSTOM_LOCATIONS
    ...
    def test_CustomPointAltitudeMethodException(self):
        def code15():
            ...
            try:
                pointDef: "CoveragePointDefinition" = covDef.point_definition
                pointDef.point_location_method = COVERAGE_POINT_LOCATION_METHOD.COMPUTE_BASED_ON_RESOLUTION

                pointDef.point_altitude_method = COVERAGE_POINT_ALTITUDE_METHOD.FILE_VALUES
