...
def all_dilutions_of_precision_time_xy_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None, step=60):
	"""A plot of all DOP values, over time, for the point currently selected via the figure of merit grid inspector. """
	root = stk_obj.root
...
def point_fom_value_time_xy_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None, step=60):
	"""A plot of the figure of merit values over time, for the point currently selected via the figure of merit grid inspector. """
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def point_satisfaction_intervals_interval_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None):
	"""An Interval graph of the satisfaction intervals for the point currently selected via the figure of merit grid inspector. Satisfaction intervals are defined as periods when a grid point achieves the defined satisfaction criteria associated with the FOM. """
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
	...
	elements=[(('interval start', '0'),('interval end', '0'))]
...
def regional_fom_values_time_xy_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None, step=60):
	"""A plot of the minimum, maximum, and average figure of merit value, sampled over all grid points within the region currently selected in the figure of merit grid inspector, over time. Grid points for which a value cannot be computed are not included in the reported statistics. """
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def periods_of_partial_regional_satisfaction_interval_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None):
	"""An Interval graph of the intervals of time when the region selected by the grid inspector is partially covered. A region is considered to be covered if at least one point within the region has access to one or more of the assigned assets. """
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def overall_grid_statistics_time_xy_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None, step=60):
	"""A plot of the miniumum, maximum, and average figure of merit values, sampled over all grid points, over time. Grid points for which a value cannot be computed are not included in the reported statistics."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def satisfied_by_time_time_xy_graph(stk_obj :FigureOfMerit, start_time=None, stop_time=None, step=60):
	"""A plot of the percentage of the grid which satisfies the satisfaction criteria defined in the figure of merit, as a function of time."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
