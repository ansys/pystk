...
def inertial_position_velocity_time_xy_graph(stk_obj :Satellite, start_time=None, stop_time=None, step=60):
	"""The position and velocity of the object with respect to the object's central body, as observed from its central body's inertial coordinate system, expressed in Cartesian components as a function of time."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def lifetime_space_weather_time_xy_graph(stk_obj :Satellite, start_time=None, stop_time=None, step=60):
	"""A plot of the space weather (F10.7 and Ap values) used when computing the object's lifetime using the Lifetime Tool."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def lifetime_time_xy_graph(stk_obj :Satellite, start_time=None, stop_time=None, step=60):
	"""A plot of the apogee height, perigee height, and eccentricity of the object over time, as computed using the Lifetime Tool."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def eci_vvlh_solar_aer_time_xy_graph(stk_obj :Satellite, start_time=None, stop_time=None, step=60):
	"""A plot of the solar elevation over time, describing the apparent relative position vector of the Sun with respect to the object. The elevation angle is measured from the XY plane of the object's body axes, positive in the +Z direction."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def teme_residuals_for_tle_fit_time_xy_graph(stk_obj :Satellite, start_time=None, stop_time=None, step=60):
	"""A plot of the final residuals, computed between the object's position and the position created using the solved-for TLE created by the Generate TLE tool, as computed in the TEME coordinate systrem, as a function of time."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
