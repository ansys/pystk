...
def cumulative_sunlight_cumulative_pie_graph(stk_obj :Ship, start_time=None, stop_time=None):
...
def eclipse_times_interval_graph(stk_obj :Ship, start_time=None, stop_time=None):
...
def lasercat_clear_firing_interval_graph(stk_obj :Ship, start_time=None, stop_time=None):
    r"""Createn Interval graph of the time intervals in which the object can communicate with the target without any unintended lasing of other satellites, as computed by the LaserCAT tool.

    This graph wrapper was generated from AGI\STK12\STKData\Styles\Ship\LaserCATClearFiring.rsg.
    """
    root = stk_obj.root
    if start_time is None:
        start_time = root.current_scenario.start_time
...
def lasercat_potential_victim_interval_graph(stk_obj :Ship, start_time=None, stop_time=None):
    r"""Createn Interval graph of the time intervals during which potential victims (i) may interfere with the communication link or (ii) be subject to interference from the communications link, as computed by the LaserCAT tool. These intervals occur during times when the object can communicate with the target.

    This graph wrapper was generated from AGI\STK12\STKData\Styles\Ship\LaserCATPotentialVictim.rsg.
    """
    root = stk_obj.root
    if start_time is None:
        start_time = root.current_scenario.start_time
...
def lighting_times_interval_graph(stk_obj :Ship, start_time=None, stop_time=None):
...
def lla_position_time_xy_graph(stk_obj :Ship, start_time=None, stop_time=None, step=60):
...
def model_area_time_xy_graph(stk_obj :Ship, start_time=None, stop_time=None, step=60):
...
def rfi_potential_victim_interval_graph(stk_obj :Ship, start_time=None, stop_time=None):
    r"""Createn Interval graph of the time intervals during which potential victims (i) may interfere with the communication link or (ii) be subject to interference from the communications link, as computed by the Radio Frequency Interference (RFI) Analysis tool. These intervals occur during times when the object can communicate with the target.

    This graph wrapper was generated from AGI\STK12\STKData\Styles\Ship\RFIPotentialVictim.rsg.
    """
    root = stk_obj.root
    if start_time is None:
        start_time = root.current_scenario.start_time
...
def ecf_vvlh_solar_aer_time_xy_graph(stk_obj :Ship, start_time=None, stop_time=None, step=60):
    r"""Create plot of the azimuth, elevation, and range over time, describing the apparent relative position vector of the Sun with respect to Fixed VVLH axes (ECFVVLH).
...
def solar_az_el_polar_center_0_graph(stk_obj :Ship, start_time=None, stop_time=None, step=60):
    r"""Create polar plot with elevation as radius and azimuth as angle theta over time, describing the apparent relative position vector of the Sun with respect to Fixed VVLH axes (ECFVVLH).
...
def sun_vector_time_xy_graph(stk_obj :Ship, start_time=None, stop_time=None, step=60):
    ...
    return line_chart_time_x(df, root, ['x','y','z'], ['time'], axes, 'Sun Vector')

def sunlight_intervals_interval_pie_graph(stk_obj :Ship, start_time=None, stop_time=None):
