...
def angle_between_data_time_xy_graph(stk_obj :Chain, start_time=None, stop_time=None, step=60):
	"""A plot of the angle and ranges for each three object sub-strand of a Chain, as a function of time, for each strand access interval that overlaps with the requested report time intervals. """
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def bent_pipe_c_no_link_analysis_time_xy_graph(stk_obj :Chain, start_time=None, stop_time=None, step=60):
	"""This graph shows the carrier-to-noise density ratio for uplink, downlink, and composite link as a function of time for a bent pipe communications system."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def complete_chain_access_times_interval_graph(stk_obj :Chain, start_time=None, stop_time=None):
	"""An Interval plot of the time intervals for which the chain is completed. These intervals are computed by overlapping all the strand accesses."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def object_access_interval_graph(stk_obj :Chain, start_time=None, stop_time=None):
	"""An Interval graph of the time intervals for each object in a Chain for which the object participates in a strand that completes the chain. Each object's intervals are graphed on a separate line."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def strand_access_times_interval_graph(stk_obj :Chain, start_time=None, stop_time=None):
	"""An Interval graph of the time intervals for each strand in a Chain that completes the chain. Each strand's intervals are graphed on a separate line."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
...
def number_of_accesses_time_xy_graph(stk_obj :Chain, start_time=None, stop_time=None, step=60):
	"""A plot of the number of objects participating in a strand that completes the chain at the given time, as a function of time. The report is only valid for Chains consisting of two objects."""
	root = stk_obj.root
	if start_time is None:
		start_time = root.current_scenario.start_time
	if stop_time is None:
