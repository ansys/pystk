...
def centroid_solar_az_el_time_xy_graph(stk_obj :AreaTarget, start_time=None, stop_time=None, step=60):
    r"""Create plot of the elevation and azimuth over time, describing the relative position vector of the apparent Sun to the area target centroid, with respect to the local horizontal plane. This frame has the Z axis aligned with the inward surface normal direction (minus Z is up) and the X axis constrained toward the local north direction.
    ...
    df = stk_obj.data_providers.item('Centroid Lighting AER').exec(start_time, stop_time, step).data_sets.to_pandas_dataframe()
    ...
    return line_chart_time_x(df, root, ['azimuth','elevation'], ['time'], axes, 'Centroid Solar Az-El')
