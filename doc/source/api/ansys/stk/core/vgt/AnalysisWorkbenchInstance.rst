AnalysisWorkbenchInstance
=========================

.. py:class:: ansys.stk.core.vgt.AnalysisWorkbenchInstance

   Bases: :py:class:`~ansys.stk.core.vgt.IAnalysisWorkbenchContext`

   Enable to obtain information about the parent object that owns the VGT component.

.. py:currentmodule:: AnalysisWorkbenchInstance

Overview
--------

.. tab-set::

    .. tab-item:: Properties
        
        .. list-table::
            :header-rows: 0
            :widths: auto

            * - :py:attr:`~ansys.stk.core.vgt.AnalysisWorkbenchInstance.instance_path`
              - Returns a path to the parent object that owns the VGT component.
            * - :py:attr:`~ansys.stk.core.vgt.AnalysisWorkbenchInstance.template`
              - Returns a template object the VGT component was created from or null if the VGT component was not created from a template.



Import detail
-------------

.. code-block:: python

    from ansys.stk.core.vgt import AnalysisWorkbenchInstance


Property detail
---------------

.. py:property:: instance_path
    :canonical: ansys.stk.core.vgt.AnalysisWorkbenchInstance.instance_path
    :type: str

    Returns a path to the parent object that owns the VGT component.

.. py:property:: template
    :canonical: ansys.stk.core.vgt.AnalysisWorkbenchInstance.template
    :type: IAnalysisWorkbenchComponent

    Returns a template object the VGT component was created from or null if the VGT component was not created from a template.


