Coverage3dDrawAtAltitudeMode
============================

.. py:class:: ansys.stk.core.stkobjects.Coverage3dDrawAtAltitudeMode

   IntEnum


.. py:currentmodule:: Coverage3dDrawAtAltitudeMode

Overview
--------

.. tab-set::

    .. tab-item:: Members
        
        .. list-table::
            :header-rows: 0
            :widths: auto

            * - :py:attr:`~FRONT_FACING`
              - Draw front facing only.

            * - :py:attr:`~BACK_FACING`
              - Draw back facing only.

            * - :py:attr:`~FRONT_AND_BACK_FACING`
              - Draw front and back facing.


Import detail
-------------

.. code-block:: python

    from ansys.stk.core.stkobjects import Coverage3dDrawAtAltitudeMode


