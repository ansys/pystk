ISRPModelSpherical
==================

.. py:class:: ansys.stk.core.stkobjects.ISRPModelSpherical

   ISRPModelBase
   
   Spherical Solar Radiation Pressure Model.

.. py:currentmodule:: ISRPModelSpherical

Overview
--------

.. tab-set::

    .. tab-item:: Properties
        
        .. list-table::
            :header-rows: 0
            :widths: auto

            * - :py:attr:`~ansys.stk.core.stkobjects.ISRPModelSpherical.cr`
            * - :py:attr:`~ansys.stk.core.stkobjects.ISRPModelSpherical.area_mass_ratio`


Import detail
-------------

.. code-block:: python

    from ansys.stk.core.stkobjects import ISRPModelSpherical


Property detail
---------------

.. py:property:: cr
    :canonical: ansys.stk.core.stkobjects.ISRPModelSpherical.cr
    :type: float

    Solar radiation pressure coefficient. Dimensionless.

.. py:property:: area_mass_ratio
    :canonical: ansys.stk.core.stkobjects.ISRPModelSpherical.area_mass_ratio
    :type: float

    Gets or sets the area-to-mass ratio to be used in SRP calculations. Uses AreaPerMass Dimension.


