...
.. tab-set::
    ...
    .. tab-item:: Members
        ...
        .. list-table::
            ...
            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_NORMAL`
              - The basic stopping conditions will be used.

            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_MIN_RANGE_FOR_EQUAL_SPEED`
              - Stop when the aircraft achieves the range for equal speed.

            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_TRANSITION_SPEED_RANGE`
              - Stop when the aircraft achieves the range to transition speed.

            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_AFTER_TARGET_CURRENT_PROCEDURE`
              - Stop after the target completes the current procedure.

            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_AFTER_TARGET_CURRENT_PHASE`
              - Stop after the target completes the current phase.

            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_WHEN_TARGET_PERF_MODE_CHANGES`
              - Stop when the target enters a new mode of flight.

            * - :py:attr:`~REL_SPEED_ALTITUDE_STOP_WHEN_TARGET_PHASE_OF_FLIGHT_CHANGES`
