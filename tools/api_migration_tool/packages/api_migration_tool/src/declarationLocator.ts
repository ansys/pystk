...
import { ReferencesProvider, ReferencesResult } from 'pyright-internal/languageService/referencesProvider';
...
export class DeclarationLocator {
    ...
    constructor(
        ...
        const documentSymbols = documentSymbolProvider.getSymbols() as DocumentSymbol[];

        this.flatDocumentSymbols = DeclarationLocator.flattenDocumentSymbols(documentSymbols);
    }

    public findDeclaration(symbolName: string, parentScope: string, category: string): ReferencesResult | undefined {
    ...
    private static findSymbol(flatDocumentSymbols: FlatDocumentSymbol[], name: string, parentScope: string, category: string): FlatDocumentSymbol | undefined {
        for (const symbol of flatDocumentSymbols) {
            if (
                symbol.name == name &&
                ((!parentScope && !symbol.containerName) || parentScope == symbol.containerName) &&
    ...
    private static flattenDocumentSymbolsRecursive(symbolList: FlatDocumentSymbol[], symbol: DocumentSymbol, path?: string) {
        ...
        if (symbol.children) {
            ...
            for (const child of symbol.children) {
                DeclarationLocator.flattenDocumentSymbolsRecursive(symbolList, child, currentPath);
            }
        }
    }
}
