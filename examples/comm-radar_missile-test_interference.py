...
# +
from ansys.stk.core.stkobjects import STKObjectType, Missile
...
# +
from ansys.stk.core.stkobjects import VehicleLaunchControl, PropagatorType, IPropagator, VehicleImpactLocationPoint
...
ground_range_report = provider.execute(scenario.start_time, scenario.stop_time, time_step).data_sets.to_pandas_dataframe()
...
missile_transmitter: Transmitter = Test_Missile.children.new(STKObjectType.TRANSMITTER, "Missile_Tx")
# -
...
# +
from ansys.stk.core.stkobjects import PropagatorType, IGreatArcVehicle, PropagatorGreatArc, VehicleWaypointComputationMethod
...
# +
from ansys.stk.core.stkobjects import TrackMode, BoresightType

antenna_motor.common_tasks.set_pointing_targeted_tracking(TrackMode.RECEIVE, BoresightType.ROTATE, "Missile/Test_Missile")
# -
...
helixModel: AntennaModelHelix = receiver_model.antenna_control.embedded_model_component_linking.component
...
link_budget_report: DataFrame = provider.execute(scenario.start_time, scenario.stop_time, time_step).data_sets.to_pandas_dataframe()
...
command: str = f"ImportFromDB * Satellite {file_location} Propagate On CommonName SXM-10"
...
for satellite in [sxm_8, sxm_9, sxm_10]:
    ...
    transmitter = ISTKObject(satellite).children.new(
        STKObjectType.TRANSMITTER,
        "Transmitter"
    )

    transmitter.model_component_linking.set_component(
...
# +
from ansys.stk.core.stkobjects import RFInterference, ISTKObject

interference = RFInterference(ship_receiver.model_component_linking.component.interference)
...
ship_receiver_filter: RFFilterModelButterworth = receiver_model.filter_component_linking.component
...
# +
from ansys.stk.core.stkobjects import AntennaModelExternal, AntennaControl
antenna_control: AntennaControl = ship_radar.model_component_linking.component.antenna_control
antenna_control.embedded_model_component_linking.set_component("External Antenna Pattern")

external_model = AntennaModelExternal(antenna_control.embedded_model_component_linking.component)
...
# +
external_model.filename = str(Path(r"C:\Program Files\AGI\STK_ODTK 13\Data\Resources\stktraining\samples\ASR9Low.pattern"))
# -
...
from ansys.stk.core.stkobjects import RadarCrossSectionComputeStrategyExternalFile, RadarCrossSectionFrequencyBand

rcs_band: RadarCrossSectionFrequencyBand = Test_Missile.radar_cross_section.model_component_linking.component.frequency_bands.item(0)
rcs_band.set_compute_strategy("External File")

external_file_compute_strategy: RadarCrossSectionComputeStrategyExternalFile = rcs_band.compute_strategy

external_file_compute_strategy.filename = str(Path(r"C:\Program Files\AGI\STK_ODTK 13\Data\Resources\stktraining\samples\Basic_Missile_mono.rcs"))
root.save_scenario()
# -
...
aer_and_search_track_data: DataFrame = provider.execute(scenario.start_time, scenario.stop_time, time_step).data_sets.to_pandas_dataframe()
...
# +
root.units_preferences.item("Distance").set_current_unit("km") # The script report is now using the same units as the scenario report.

provider = access.data_providers.item("Radar SearchTrack")
aer_and_search_track_data: DataFrame = provider.execute(scenario.start_time, scenario.stop_time, time_step).data_sets.to_pandas_dataframe()
print(aer_and_search_track_data[["s/t integrated snr", "s/t integrated pdet", "s/t pulses integrated"]])
# -
