...
def random_dates(launch_dates, arrival_dates):
    return float(random.choice(launch_dates + arrival_dates).format("JDate"))


def fitness_function(individual):
    launch, arrival = [
        root.root.conversion_utility.new_date("JDate", str(date)) for date in individual
    ]

    # Death sentence
    is_valid_launch = (
        launch_span[0].whole_days_utc
        <= launch.whole_days_utc
        <= launch_span[-1].whole_days
    )
    is_valid_arrival = (
        arrival_span[0].whole_days_utc
        <= arrival.whole_days_utc
        <= arrival_span[-1].whole_days
    )
    if not is_valid_launch or not is_valid_arrival:
        return (1e6,)

    dv_launch, _, _ = lambert_solver(satellite, earth, mars, launch, arrival)
...
# Declare the type of problem
creator.create("FitnessMin", base.Fitness, weights=(-1.0,))
creator.create("Individual", list, fitness=creator.FitnessMin)

# Declare the different algorithms
toolbox = base.Toolbox()

# Gene generation based on random dates
toolbox.register("attr_uniform", random_dates, launch_span, arrival_span)
toolbox.register(
    "individual", tools.initRepeat, creator.Individual, toolbox.attr_uniform, 2
)
toolbox.register("population", tools.initRepeat, list, toolbox.individual, 75)
...
best = hall_of_fame[0]
best = [root.conversion_utility.new_date("JDate", str(date)) for date in best]
# -

print(f"Best launch date: {best[0].format('UTCG')}")
print(f"Best arrival date: {best[1].format('UTCG')}")
